...
public class UT_Utils extends TestUnit {
	...
	{
		AssertEquals(new Character(Utils.ASCII(0)),new Character('0'),"ASCII(0)");
		AssertEquals(new Character(Utils.ASCII(0xa)),new Character('A'),"ASCII(A)");
		AssertEquals(new Character(Utils.ASCII(0xf)),new Character('F'),"ASCII(F)");
	...
	boolean GetIndexTest() throws AssertException
	{
		...
		int agentIndex=Utils.getIndex(payload,Conf.AGENT_CONF_DELIMITER.getBytes());
		int eventIndex=Utils.getIndex(payload,Conf.EVENT_CONF_DELIMITER.getBytes());
		int mobileIndex=Utils.getIndex(payload,Conf.MOBIL_CONF_DELIMITER.getBytes());
		int actionIndex=Utils.getIndex(payload,"SICURAMENTENONCE".getBytes());
		int endofIndex=Utils.getIndex(payload,Conf.ENDOF_CONF_DELIMITER.getBytes());
	...
	boolean CrcTest() throws AssertException
	{
		...
		int[] expects=new int[]{0x0,0x1,0x4,0xf,0x40,0x145,0x7a4,0x3583,0x1ac20,0xf0d29};
	    for (int i = 0; i < 10; i++) {
		        results[i] = Utils.crc(buffer,0, i);
		        AssertEquals(expects[i],results[i],"CRC2 "+i);
	    }
		
	    expects=new int[]{0x0,0x9683a4,0xfb58214c,0x598075bf,0x9d9667b9,0x8ed1cd81,0x7493338,0x7f3e6d8f,0xc318e3b3,0x77617634};
	    for (int i = 0; i < 10; i++) 
	...
	public void IntToByteTest() throws AssertException, IOException
	{
		...
		for(int i = Integer.MIN_VALUE; i<= Integer.MAX_VALUE / 2; i+= random.nextInt(Integer.MAX_VALUE / 5))
		{
			byte[] arr = Utils.intToByteArray(i);
	...
	private void DateTimeTest() {
		...
		Check.asserts(calendar.get(Calendar.MONTH) == Calendar.MARCH, "Wrong Month");
